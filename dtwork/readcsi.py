...
def read_log_file(filename, object_type, payload_on, filter_payload=True):
    ...
    with open(filename, 'rb') as f:
        ...
        while cur < (len_file - 4):
            ...
            csi_matrix = {}
            csi_matrix['timestamp'], csi_matrix['csi_len'], csi_matrix['tx_channel'], csi_matrix['err_info'], csi_matrix['noise_floor'], csi_matrix['rate'], csi_matrix['bandWitdh'], csi_matrix[
                'num_tones'], csi_matrix['nr'], csi_matrix['nc'], csi_matrix['rssi'], csi_matrix['rssi1'], csi_matrix['rssi2'], csi_matrix['rssi3'], csi_matrix['payload_len'] = unpack('>QHHBBBBBBBBBBBH', f.read(25))
            cur += 25

            if csi_matrix['csi_len']:
                # This condition saves a decent amount of time
                csi_buf = unpack(
                    'B' * csi_matrix['csi_len'], f.read(csi_matrix['csi_len']))
                csi_matrix.update(
                    read_csi(csi_buf, csi_matrix['nr'], csi_matrix['nc'], csi_matrix['num_tones']))
...
def make_abs_csi_dfs(complex_csi_dfs):
    ...
    for one_df in complex_csi_dfs:
        abs_csi_dfs.append(one_df.drop(columns=['object_type']).abs().assign(
            object_type=one_df['object_type'].values))
...
def make_phase_csi_dfs(complex_csi_dfs):
    ...
    for one_df in complex_csi_dfs:
        ...
        phase_csi_dfs.append(pd.DataFrame(np.angle(tmp_df.values), columns=tmp_df.columns).assign(
            object_type=one_df['object_type'].values))
